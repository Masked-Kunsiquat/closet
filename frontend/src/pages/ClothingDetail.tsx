...
import { getClothingItemById } from "../api/clothing";
...
export default function ClothingDetail() {
  ...
  useEffect(() => {
    if (id) {
      getClothingItemById(id).then(setItem);
    }
  }, [id]);
      ...
      <div className="py-8 px-4 mx-auto max-w-2xl lg:py-16">
        ...
        <dl>
          ...
          <dd className="mb-4 font-light text-gray-500 sm:mb-5 dark:text-gray-400">
            ...
            Size: {item.size}.
            {item.purchase_date && ` Purchased on: ${new Date(item.purchase_date).toLocaleDateString()}`}
          </dd>
        </dl>
        ...
        <Link to="/clothes" className="block text-blue-500 mt-4">
          ← Back to Clothes
        </Link>
      </div>
    </section>
